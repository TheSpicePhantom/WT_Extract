"""Demo: Tile-Layer — instanced 32×32 Tiles aus TileLayerBatch (wt:… Keys)."""
...
from render_graphics.tile_layer import demo_tile_chunk_with_pond
from render_scene.types import RenderFrame, TILE_SIZE_PX

MOVE_SPEED = 480.0
ZOOM_SPEED = 1.8
WINDOW_TITLE_BASE = "WT Extract — Tile Layer (GPU)"
...
# Dekoration über dem Tile-Layer
DECORATION_KEYS = (
...
def main() -> int:
    assert_gpu_only("tile_layer_demo")
    ...
    tile_chunk = demo_tile_chunk_with_pond(
        catalog,
        cols=64,
        rows=48,
        origin_x=-float(TILE_SIZE_PX) * 32,
        origin_y=-float(TILE_SIZE_PX) * 24,
    )

    sprites = demo_sprite_field(
        cols=3,
        rows=1,
        spacing=float(TILE_SIZE_PX) * 8,
        origin_x=-float(TILE_SIZE_PX) * 20,
        origin_y=-float(TILE_SIZE_PX) * 8,
        sprite_keys=DECORATION_KEYS,
        catalog=catalog,
    )
        ...
        while not window.should_close:
            ...
            if fps_counter.tick(dt):
                glfw.set_window_title(
                    ...
                    f"{len(tile_chunk.layers[0].tile_ids)} tiles",
                )
    finally:
        renderer.destroy()
        window.destroy()
